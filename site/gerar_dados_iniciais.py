...
for id_usr in range(2,12):
    ...
    saida.write(clube.format(id_usr, id_usr, nome_c, random.randint(5,10)*10, id_usr%4) + '\n')
for id_usr in range(2,12):
    for i in range(20):
        saida.write(jogador.format(id_usr*20 + i, id_usr, nomes[id_usr*20 +i - 40], random.randint(1,10), i%4) + '\n')
    for i in range(5):
